...
Original file is located at
    https://colab.research.google.com/drive/1O5y0e9TSDuvzoOcyF9lVLx9QjEWKhMdK
...
def reduce_word_index_features(word_index, reverse_word_index):
...
def sequence_padding(train_data, test_data):
...
def run_deeplearning():
    ...
    if embeddings_mode == "One-hot Encoding":  
        ...
        word_index, reverse_word_index = reduce_word_index_features(word_index, reverse_word_index)  # Update the word index to match the number of features we selected       
        ...
        train_data, test_data = sequence_padding(train_data, test_data)
        # NEW
        test_sentence, _ = sequence_padding(test_sentence, test_sentence) 
    ...
    print()
    print(type(test_sentence[0]), list(test_sentence[0]))
...
# Evaluation Phase
def evaluate_single_sentence(model, test_sentence):
    if multiclass == False: 
        predictions = [1 * (x[0]>=0.5) for x in model.predict(test_sentence)]
    else:    
        predictions = np.argmax(model.predict(test_sentence), axis=1)
    ...
    print(evaluate_single_sentence(model, test_sentence))
    return(evaluate_single_sentence(model, test_sentence))